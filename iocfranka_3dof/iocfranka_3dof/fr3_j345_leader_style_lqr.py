...
class FR3J345LeaderStyleLQR(Node):
    def __init__(self):
        super().__init__('fr3_j345_leader_style_lqr')
        ...
        self.declare_parameter('tau_limit', 10.0)
        ...
        # Handles prefixes like leader_fr3_, follower_fr3_, clone_fr3_, leader2_fr3_
        if prefix.endswith('fr3_'):
            self.j3 = f'{prefix}joint3'
            self.j4 = f'{prefix}joint4'
            self.j5 = f'{prefix}joint5'
        else:
            self.j3 = f'{prefix}fr3_joint3'
            self.j4 = f'{prefix}fr3_joint4'
            self.j5 = f'{prefix}fr3_joint5'

        q3 = float(self.get_parameter('q_des_j3').value)
        q4 = float(self.get_parameter('q_des_j4').value)
        q5 = float(self.get_parameter('q_des_j5').value)

        self.q_des = np.array([q3, q4, q5], dtype=float)
        self.dq_des = np.zeros(3, dtype=float)
        self.x_eq = np.hstack((self.q_des, self.dq_des))
        self.u_eq = np.zeros(3, dtype=float)
        ...
        self.get_logger().info('========== FR3 J345 LEADER-STYLE LQR ==========')
