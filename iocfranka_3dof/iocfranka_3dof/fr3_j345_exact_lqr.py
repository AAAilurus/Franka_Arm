...
class FrankaJ345ExactLQR(Node):
    def __init__(self):
        ...
        self.declare_parameter('hold_pos_tol', 0.03)
        self.declare_parameter('hold_vel_tol', 0.08)
        self.declare_parameter('hold_kp', 4.0)
        self.declare_parameter('hold_kd', 1.5)
        ...
        self.hold_pos_tol = float(self.get_parameter('hold_pos_tol').value)
        self.hold_vel_tol = float(self.get_parameter('hold_vel_tol').value)
        ...
        self.u_eq = np.zeros(3, dtype=float)  # Gazebo Franka plugin already adds gravity compensation
        ...
        self.get_logger().info('========== FR3 3DOF MATLAB-style LQR ==========')
        ...
        self.get_logger().info(f'hold_pos_tol      = {self.hold_pos_tol}')
        self.get_logger().info(f'hold_vel_tol      = {self.hold_vel_tol}')
        self.get_logger().info(f'hold_kp/hold_kd   = {self.hold_kp}, {self.hold_kd}')
        self.get_logger().info(f'hold_tau_limit    = {self.hold_tau_limit}')
    ...
    def loop(self):
        ...
        # If close to the target and nearly stopped, switch to soft hold mode.
        # This prevents the effort controller from continuously "searching".
        if (np.linalg.norm(e) < self.hold_pos_tol) and (np.linalg.norm(dq) < self.hold_vel_tol):
            self.in_hold_mode = True

        if self.in_hold_mode:
            # Hold mode: small PD torque around target.
            # Gazebo/franka sim already provides gravity behavior, so u_eq can stay zero.
            tau = -self.hold_kp * e - self.hold_kd * dq
            tau = np.clip(tau, -self.hold_tau_limit, self.hold_tau_limit)
        else:
            # Move mode: LQR feedback.
